        ...
        if (preference == mCustomWeatherLoc) {
            ...
            okButton.setOnClickListener(new View.OnClickListener() {
                ...
                private CharSequence[] buildItemList(List<LocationResult> results) {
                    ...
                    String countryCode = results.get(0).countryCode;
                    String postalCity = null, postalCountryCode = null;

                    for (LocationResult result : results) {
                        if (!TextUtils.equals(result.countryCode, countryCode)) {
                            needCountry = true;
                        }
                        if (TextUtils.equals(result.city, postalCity)
                                && TextUtils.equals(result.countryCode, postalCountryCode)) {
                            needPostal = true;
                        }
                        if (postalCity == null) {
                            postalCity = result.city;
                            postalCountryCode = result.countryCode;
                        }
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        if (needCountry) {
                            builder.append(" (").append(result.country).append(")");
                        }
