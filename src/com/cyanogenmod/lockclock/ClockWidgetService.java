...
public class ClockWidgetService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (intent.getBooleanExtra(Constants.REFRESH_CLOCK, false)) {
                if (DEBUG) Log.d(TAG, "Forcing a clock refresh");
                mRefreshClock = true;
            }
    ...
    private void refreshWidget() {
        ...
        // Refresh the Clock widget if the configuration was changed
        if (mRefreshClock) {
            refreshClock(remoteViews, digitalClock);
            mRefreshClock = false;
        }

        // Always refresh the alarm
    ...
    private long calculateUpdateTime(long minInterval) {
        ...
        // See if an event expires earlier
        for (EventInfo event : mCalendarInfo.mEventsList) {
        ...
        Log.i(TAG, "It is now = " + now + ", next update at = " + (now + minUpdateTime) + ", interval = " + minInterval);

        return now + minUpdateTime;
    }

    private long getNextWeatherUpdateTime(long now, long minInterval) {
        return (now - mWeatherInfo.last_sync) + minInterval;
    }
    ...
    private void scheduleUpdate(boolean includeCalendar) {
        ...
        am.cancel(pi);
        am.setRepeating(AlarmManager.RTC, updateTime, minInterval, pi);
    }
}
